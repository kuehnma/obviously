...
obvious::Matrix RansacMatching::match(obvious::Matrix* M, obvious::Matrix* S,
	...
	if ((indices.size()) < sizeControlSet) {
		LOGMSG(DBG_DEBUG,
				"Size of scene smaller than control set ... reducing size");
	...
	unsigned int ctr = 0;
	for (vector<unsigned int>::iterator it = idxControl.begin();
			it != idxControl.end(); ++it) {
	...
	LOGMSG(DBG_DEBUG,
			"Scene size: " << S->getRows() << ", Control set: " << idxControl.size());
...
#pragma omp parallel
{
	...
	LOGMSG(DBG_DEBUG,
			"Matching result - cnt(best): " << cntBest << ", err(best): " << errBest);

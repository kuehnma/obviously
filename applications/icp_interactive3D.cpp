...
int main(int argc, char* argv[]) {
	...
	if (argc != 6) {
		std::cout << "Usage: " << argv[0]
				<< " <model> <scene> <pre> <post> <est>" << std::endl;
		return EXIT_FAILURE;
	}
	...
	_vmodel->copyData(model->getCoords(), model->getNormals(),
			model->getColors());
	CartesianCloud3D* scene = new CartesianCloud3D(_vscene->getSize(), true);
	_vscene->copyData(scene->getCoords(), scene->getNormals(),
			scene->getColors());
	...
	PairAssignment* assigner = (PairAssignment*) new FlannPairAssignment(3,
			0.0);
	...
	double P[12] = { 585.05108211, 0.00000000, 315.83800193, 0., 0.00000000,
			585.05108211, 242.94140713, 0., 0.00000000, 0.00000000, 1.00000000,
			0. };
	...
	IPostAssignmentFilter* filterD =
			(IPostAssignmentFilter*) new DistanceFilter(1.5, 0.03, iterations);
	if (post == 1)
...
void iterateCallback() {
	...
	cout << "ICP state: " << state << ", elapsed (" << iterations
			<< " iterations): " << timer.elapsed() << " s , rms = " << rms
			<< ", # of pairs: " << pairs << endl;
...
void stepCallback() {
	...
	Timer timer;
	timer.start();
	EnumState state = _icp->step(&rms, &pairs);
	cout << "ICP state: " << state << ", elapsed (step): " << timer.elapsed()
			<< " s , rms = " << rms << ", # of pairs: " << pairs << endl;

...
int main(int argc, char** argv) {
	...
	obvious::Matrix T = MatrixFactory::TransformationMatrix33(deg2rad(9.0), 0.4,
			0.35);
	...
	PairAssignment* assigner = (PairAssignment*) new FlannPairAssignment(2, 0.0,
			true);
	//PairAssignment* assigner       = (PairAssignment*)  new NaboPairAssignment(2);
	IPostAssignmentFilter* filterD =
			(IPostAssignmentFilter*) new DistanceFilter(1.5, 0.01, iterations);
	...
	IRigidEstimator* estimator =
			(IRigidEstimator*) new PointToLine2DEstimator();

	Icp* icp = new Icp(assigner, estimator);
	icp->activateTrace();
	icp->setModel(M, N);
	icp->setScene(&S, NULL);
	int loadStatus = icp->loadParametersFromXML(
			"/home/mkuehn/conf/icpConf.reg");
	assert(loadStatus == 0);
	cout << "Setting MaxRMS to " << icp->getMaxRMS() << endl;
	cout << "Setting Iterations to " << icp->getMaxIterations() << endl;
	cout << "Setting ConvCnt to " << icp->getConvergenceCounter() << endl;
